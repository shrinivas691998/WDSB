...
export function DemandForm({ onSubmit, onCancel, initialData }: DemandFormProps) {
  const { register, handleSubmit, formState: { errors } } = useForm<Demand>({
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-800 p-6 rounded-lg shadow-lg max-w-md mx-auto">
      <div className="space-y-4">
        <div>
          ...
          <input
            {...register('demandNo', { required: 'Demand No. is required' })}
          ...
          />
          {errors.demandNo && (
            <p className="mt-1 text-sm text-red-400">{errors.demandNo.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.demandDate && (
            <p className="mt-1 text-sm text-red-400">{errors.demandDate.message}</p>
          )}
        </div>
